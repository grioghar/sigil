...
| **0.9** | **Language surface complete.** Generics over functions/records/enums, length + payload verification, the full toolchain (interpreter, Z3 verifier, native compilation via rustc, canonical form, JSON API, modules). Two real programs proven (task tracker 13/13, JSON parser 49/49). Conformance-tested (390 tests). Rich, but the toolchain still runs on Python. | **done** |
| **1.0** | **Self-hosting.** The Sigil compiler is written *in Sigil* and compiles itself — the bar for 1.0. A language is not 1.0 until it compiles with its own toolchain. The bootstrap proceeds component by component (lexer → parser → checker → emitter), each written in Sigil, compiled by the current toolchain, and conformance-tested against its Python counterpart, until the Sigil-written compiler reproduces itself. Self-hosting will itself drive missing language features (command-line arguments, a `Map`, …) — the compiler is the ultimate dogfood. **Achieved:** cc0 (a Sigil compiler in Sigil) compiles its own source to a static x86-64 ELF that recompiles cc0 byte-identically, with no rustc/LLVM/assembler/linker/libc on the compile path. | **done** |
| **1.1** | **Architecture-specific code optimization.** The 1.0 cc0 backend is a correct but naïve stack machine — every value spills to the frame, every binop round-trips through `push`/`pop`, `push` (the list op) copies the whole list (O(n²)), no folding, no use of the architecture's addressing modes. 1.1 makes cc0 generate *good x86-64*: an instruction-level IR between AST and bytes; register allocation (keep hot values and temporaries in registers, drop redundant spills); peephole rewriting (push/pop → mov, dead load/store elimination, `xor`-zero, `test` vs `cmp 0`); constant folding and strength reduction; x86-64-aware instruction selection (`lea` for address/affine arithmetic, `inc`/`dec`, shifts for power-of-two `*`/`/`); and a growable-list representation to kill the O(n²) `push`. The compiler optimizes its own source — the binaries it self-hosts on get measurably faster. Foundation for 1.2. Gated on 1.0 (now in hand). | |
| **1.2** | **Dogfood a video driver.** Write a real video/display driver *in Sigil*, compiled by the optimized cc0 — the first systems-level dogfood. Exercises tight, architecture-specific machine code, memory-mapped I/O, and the capability model against hardware access. Drives whatever 1.1's backend still can't express. | |
| **1.x** | **Translation into Sigil.** A front-end that ingests existing code in another language and emits Sigil — interpreting its semantics and, crucially, *reconstructing* the capabilities, effects, and contracts the source left implicit (every ambient `print`/file/socket becomes a threaded capability and a declared effect; checked preconditions become `requires`). The hard, valuable part is not syntax translation but making the source's hidden authority and assumptions explicit and machine-checkable — turning untyped, ambient-authority code into audited Sigil. Plus a dependency-free distribution (Z3 statically linked), `?`-style error propagation, a shared standard library, a `Box[T]` indirection type, an `Args` capability, a `Map`, and `Float`. | |
